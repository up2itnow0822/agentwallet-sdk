...
export const ABSTRACT_APPROVED_FACILITATORS = [
...
export class AbstractDelegatedFacilitatorAdapter {
    constructor(walletClient, config, chainId = ABSTRACT_CHAIN_IDS.mainnet) {
        ...
        const isApproved = approved.some((a) => a.toLowerCase() === config.facilitatorAddress.toLowerCase());
        if (!isApproved) {
            throw new Error(`Facilitator ${config.facilitatorAddress} is not on the Abstract approved list. ` +
    ...
    async signPaymentPermit(params) {
        ...
        const permitData = encodeAbiParameters(parseAbiParameters('address facilitator, address token, uint256 amount, address payTo, uint256 nonce, uint256 deadline, string resource, bytes signature'), [
            permit.facilitator,
            permit.token,
            permit.amount,
            permit.payTo,
    ...
    static isAbstractChain(chainId) {
        return Object.values(ABSTRACT_CHAIN_IDS).includes(chainId);
    }
    /** Get the network string for x402 payment requirements (e.g. "abstract:2741") */
    static getNetworkString(chainId) {
...
export const ABSTRACT_SUPPORTED_CHAINS = {
    'abstract:2741': {
        ...
        x402Model: 'delegated-facilitator',
