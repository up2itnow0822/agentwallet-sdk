...
import { issueCredential, verifyCredential } from '../identity/did.js';
import { AgentStakingPool } from '../staking/AgentStakingPool.js';
import type { SupportedStakingChain } from '../staking/AgentStakingPool.js';
...
function base58Encode(bytes: Uint8Array): string {
  ...
  for (let i = 0; i < bytes.length && bytes[i] === 0; i++) result.unshift('1');
  return result.join('');
}
...
function multibaseToPubkey(multibase: string): Uint8Array {
  if (!multibase.startsWith('z')) throw new Error('Expected base58btc multibase');
 ...
 */
export async function verifyAgent(params: VerifyAgentParams): Promise<AgentTrustBundle> {
  ...
  try {
    const balance = await pool.getBalance(agentDID, chain as SupportedStakingChain);
 ...
 */
export function verifyTrustBundle(bundle: AgentTrustBundle): boolean {
  try {
 ...
 */
function computeReputationScore(input: ReputationInput): number {
