...
const AAVE_POOL_ADDRESS: Record<SupportedStakingChain, Address> = {
  base: '0xA238Dd80C259a72e81d7e4664a9801593F98d1c5',
  arbitrum: '0x794a61358D6845594F94dc1DB02A252b5b4814aD',
};

const USDC_ADDRESS: Record<SupportedStakingChain, Address> = {
  base: '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913',
  arbitrum: '0xaf88d065e77c8cC2239327C5EDb3A432268e5831',
};

// aUSDC (interest-bearing USDC from AAVE)
const AUSDC_ADDRESS: Record<SupportedStakingChain, Address> = {
...
// ─── Deposit tracker (in-memory; production should use on-chain events or DB) ─
...
const depositTracker = new Map<string, DepositRecord>();

function depositKey(did: string, chain: SupportedStakingChain): string {
  return `${did}:${chain}`;
}
...
export class AgentStakingPool {
   ...
   */
  async getBalance(agentDID: AgentDID, chain: SupportedStakingChain): Promise<StakingBalance> {
    const viemChain = CHAINS[chain];
   ...
   */
  private async verifyAgentOnly(agentDID: AgentDID, walletClient: WalletClient): Promise<void> {
    if (!agentDID.did.startsWith('did:key:z6Mk')) {
      throw new Error(
        'AGENT_ONLY: Invalid AgentDID format. Only did:key (ed25519) DIDs are accepted. ' +
   ...
   */
  private calculateManagementFee(principal: bigint, daysStaked: number): bigint {
    if (daysStaked <= 0 || principal === 0n) return 0n;
